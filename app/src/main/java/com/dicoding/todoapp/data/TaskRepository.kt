...
class TaskRepository(private val tasksDao: TaskDao) {
    ...
    fun getTasks(filter: TasksFilterType): LiveData<PagedList<Task>> {
        ...
        val builderPageList = PagedList.Config.Builder().setEnablePlaceholders(PLACEHOLDERS)
            .setPageSize(PAGE_SIZE).build()

        return LivePagedListBuilder(tasksDao.getTasks(filteredQuery),builderPageList).build()
    }
